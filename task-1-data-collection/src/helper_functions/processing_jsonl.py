            ...
            with open(file_path, "r") as file:
                for line in file:
                    try:
                        ...
                        data["url"] = data.get("title", None)   
                        data["retrieved_title"] = data.get("title", None)
                        data["markdown_content"] = data.get("text", None)
...
def save_markdown_content_to_file(row, index, destination_folder, name="document"):
    """
    Saves the text content of a row to a text file in the specified folder.
    ...
    markdown_content = row["markdown_content"]  # The actual text content to be saved
    file_name = row["title"]

    # Construct the output file path in the ACT folder with a sequential name
    output_file_path = os.path.join(destination_folder, f"{file_name}.md")  # Use 1-based index for naming
    
    row['file_type'] = filename.strip................
    row['file_path'] = output_file_path
    row['markdown_path'] = output_file_path
...
if __name__ == "__main__":
    ...
    save_dataframe_to_csv(df, '/content/drive/MyDrive/Omdena_Challenge/PREPROCESSING/llamaparse/llama_processed_documents_markdown.csv')
